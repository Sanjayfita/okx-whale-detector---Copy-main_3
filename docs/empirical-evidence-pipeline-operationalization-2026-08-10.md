...
OKX public book/trades/candles
  ...
  -> OKXStreamingPriceReader using the validated public order-book stream at nine frozen horizons
...
| Outcome recovery          | `persistentOutcomeScheduler`, `liveEvidenceCollector`, `okxStreamingPriceReader`                                                                                            | Operational; nine horizons, atomic state, idempotent reconciliation, durable missed-window states, sampled raw paths, restart recovery, and no deadline-critical per-horizon REST dependency.              |
