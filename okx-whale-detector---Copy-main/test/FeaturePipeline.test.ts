...
const timestamped = (observedAt: number) => ({
  instrumentId,
  observedAt,
...
describe('calculateResearchFeatures', () => {
  it('calculates timestamp-bounded derivatives features and regime', () => {
    const result = calculateResearchFeatures({
      ...
      funding: [
        ...
        {
          ...timestamped(start + 60 * 60 * 1_000),
          observedAt: start + 170_000,
          kind: 'FUNDING',
  ...
  it('excludes future records from all calculations', () => {
    const minimal = {
      instrumentId,
      asOf: start,
      trades: [
    ...
    expect(result.aggressiveDeltaContracts).toBe(1);
  });
});
