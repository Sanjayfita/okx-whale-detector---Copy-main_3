...
export type WallDetectorBenchmarkPattern =
  | 'irrelevant'
  | 'exact-wall'
  | 'removal'
  | 'reappearance'
...
export type WallDetectorBenchmarkActiveState = 'few' | 'many';

export interface WallDetectorBenchmarkMetric {
...
export interface WallDetectorBenchmarkResult {
  readonly depthPerSide: number;
  readonly activeState: WallDetectorBenchmarkActiveState;
  readonly pattern: WallDetectorBenchmarkPattern;
  readonly wallDetector: WallDetectorBenchmarkMetric;
  readonly whaleTracker: WallDetectorBenchmarkMetric;
  readonly combinedDetectors: WallDetectorBenchmarkMetric;
  readonly marketEngine: WallDetectorBenchmarkMetric;
}

export interface WallDetectorBenchmarkOptions {
  readonly depths?: readonly number[];
  readonly activeStates?: readonly WallDetectorBenchmarkActiveState[];
  readonly patterns?: readonly WallDetectorBenchmarkPattern[];
  readonly samples?: number;
...
const DEFAULT_PATTERNS: readonly WallDetectorBenchmarkPattern[] = [
  'irrelevant',
  'exact-wall',
  'removal',
  'reappearance',
...
const isQualifyingIndex = (
  index: number,
  depth: number,
  activeState: WallDetectorBenchmarkActiveState,
...
const summarize = (values: readonly number[]): WallDetectorBenchmarkMetric => {
...
const runScenario = (
  depth: number,
  activeState: WallDetectorBenchmarkActiveState,
  pattern: WallDetectorBenchmarkPattern,
  samples: number,
  warmups: number,
): WallDetectorBenchmarkResult => {
  const removalGraceMs = pattern === 'removal' ? 0 : Number.MAX_SAFE_INTEGER;
  ...
  const processUpdate = (iteration: number, record: boolean): void => {
    ...
    switch (pattern) {
      ...
      case 'reappearance':
        removed = !removed;
        bids = [level(100, removed ? 0 : 6_000 + (iteration % 20))];
        break;
...
export const runWallDetectorBenchmark = (
  options: WallDetectorBenchmarkOptions = {},
): readonly WallDetectorBenchmarkResult[] => {
  ...
  const results: WallDetectorBenchmarkResult[] = [];
...
const formatMetric = (metric: WallDetectorBenchmarkMetric): string =>
...
export const runWallDetectorPerformanceReport = (
  args: readonly string[] = process.argv.slice(2),
): void => {
  const samples = Number(
    args.find((argument) => /^\d+$/.test(argument)) ?? DEFAULT_SAMPLES,
  );
  const results = runWallDetectorBenchmark({ samples });

  console.log('\nWALL DETECTOR ALLOCATION PERFORMANCE REPORT');
  console.log(
...
if (require.main === module) {
  try {
    runWallDetectorPerformanceReport();
  } catch (error: unknown) {
    console.error(
      'WallDetector performance report failed:',
